#!/usr/bin/env python3
"""05_8, Route C: where the two routes agree. The main result of this stage.
...
annotation to fall back on - `cell_type` is `malignant` everywhere - so a dimension that
passes only Route A cannot be sanity-checked against a lineage label; convergence with the
gene programme of Route B is what stands in for it.
...
table below is 2 x n_latent = 64 rows at this phase's latent size, and a dimension DRVI
wrote off can still be read.
