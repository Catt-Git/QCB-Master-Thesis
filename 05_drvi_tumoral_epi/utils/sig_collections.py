...
05_4 - 05_8 implement ONE procedure: score prior knowledge on the cells (Route A), read the
...
the third of which uses only the part of it that does not need a target region:
  ...
  * `scie`  - stemness x immunogenicity. Is there a MALIGNANT state that is stem-like AND
...
and carries the collection in its filename. The Benjamini-Hochberg correction of 05_7 is
...
class Derived:
    ...
    so 05_7 never sees it and 05_8 reports it as an independent Route A readout. The subtraction
...
@dataclass(frozen=True)
class Criterion:
    """One half of the "is this axis the project's target?" test in 05_8.
...
class Collection:
    ...
    # define the region, what to call it, the named risks 05_6 must check, and the
    # criteria 05_8 tests the axes against. `scie` and `emt` do.
    #
    ...
    risks: tuple[str, ...] = ()      # named checks 05_6 must run, see its header
    criteria: tuple[Criterion, ...] = ()  # the 05_8 target-axis test
    ...
    def block_edges(self, ordered: list[str]) -> list[int]:
        ...
        hypotheses, which is exactly what the Jaccard matrix of 05_4 is there to show.
        """
        edges, prev = [], None
...
SCIE = Collection(
    ...
    question="is there a malignant state that is stem-like AND immune-evasive?",
    ...
    # CytoTRACE2 is joined by 05_6 from the csv 05_5 writes, when that csv exists. It is the
    # only stemness evidence in the whole stage that does not come from a gene list, so the
    # quadrant is materially weaker without it and 05_6 says so rather than failing quietly.
...
#   * the middle band is then RECOVERED rather than assumed. On 04's epithelial compartment the
#     E-to-M score of the co-expression set fell at percentiles 38 / 50 / 63 without that ever
#     having been imposed, and the set was enriched 1.3 - 2.1x for hybrid-high cells. The hybrid
#     lists became the validation instead of the definition, which is what they are actually
#     good for.
#
# EVERY NUMBER IN THIS BLOCK WAS MEASURED ON 04's 74,441 EPITHELIAL CELLS, not on this subset.
# The definition carries over because the argument behind it does - a narrow band intersected
# with a narrow band is unstable wherever the scores are continuous - but the stability figures
# have to be re-read from this phase's own `quadrant_stability` table before being quoted of the
# tumour. 05_6 writes it.
...
def _emt_flags(coll: "Collection", claimed: str, a_rho: float) -> list[str]:
    ...
    # object had to be called aneuploid to be here, so an actual fibroblast is not among them
    # - but ambient RNA is contamination, not identity, and a tumour cell in a droplet with
    # fibroblast soup is still exactly as mesenchymal-looking as before. The flag therefore
    # stays on every mesenchymal claim, and 05_6 runs the doublet check behind it.
