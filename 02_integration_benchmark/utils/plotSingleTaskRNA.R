...
plotSingleTaskRNA <- function(csv_metrics_path, outdir = ".", weight_batch = 0.4){
  ...
  for (dt.sc in data.scenarios){
    ...
    methods_name <- plyr::mapvalues(methods_name,
                                    from = c("Seurat", "Seuratrpca", "Mnn", "Bbknn", "Trvae", "Scvi", "Liger", "Combat", "Saucie", "Fastmnn", "Desc", "Scanvi", "Scgen", "Drvi"),
                                    to = c("Seurat v3 CCA", "Seurat v3 RPCA", "MNN", "BBKNN", "trVAE", "scVI", "LIGER", "ComBat", "SAUCIE", "fastMNN", "DESC", "scANVI*", "scGen*", "DRVI"))
