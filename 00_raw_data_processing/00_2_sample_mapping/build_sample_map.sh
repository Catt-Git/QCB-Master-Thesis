...
# Two DISTINCT naming formats (do not confuse): original library_name hNN[timepoint][subnum]_[P|N] (e.g. h19C_P, h03B2_P) vs Cell Ranger sample_id PNN_[timepoint][subnum]_[P|N] (e.g. P19_C_P, P03_B2_P). 'P' = CD45+ (immune), 'N' = CD45- (non_immune). Patient 03 is split into sub-portions A1/A2/B1/B2/C1/C2 (biopsy divided).
...
# Input metadata table and output filename for the cleaned sample map. Both are small TSVs that live IN the repo (no DATA_DIR needed) and are resolved from this script's location, so it can be run from any working directory.

tail -n +2 "$META" | awk -F'\t' '{print $1"\t"$7}' \
  | grep -vP '_TCR$' \
  | grep -vP '^SRR[0-9]+\tm[A-F]' \
  > "$SCRIPT_DIR/srr_gex_only.tsv"
# Keep only run_accession (col 1) and library_name (col 7), then drop TCR/BCR libraries (_TCR suffix) and mouse samples (library_name starting with 'm' + letter A-F).
...
# Parse each library_name into patient/timepoint/subnum/fraction and rebuild the Cell Ranger sample_id; rows that do not match the expected pattern are flagged UNPARSED for manual review (in our case the 8 UNPARSED entries were mouse E0771 samples).
