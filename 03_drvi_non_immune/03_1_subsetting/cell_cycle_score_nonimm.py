"""
03_1 cell cycle scoring of the non-immune subset (mirrors 01_4/cell_cycle_score.py).
...
the 176k non-immune cells here, of 620k mixed cells there, so the same gene sets
...
inherited and sub-annotation belongs to 03_2), and `fraction_reassignment` is
meaningless here because `fraction` is constant ('non_imm') by construction.

Input : $DATA_DIR/03_nonimm/shiao_nonimm_norm.h5ad
        Output of scran_norm_nonimm.py. .X = scran log-normalized (scoring
        requires log-norm, not raw); gene symbols as var_names.
Output: $DATA_DIR/03_nonimm/shiao_nonimm_norm_cc.h5ad
...
output already exists, and tees everything to 03_1_subsetting/logs/. It also
...
python3 cell_cycle_score_nonimm.py
"""
...
IN_PATH = os.path.join(NONIMM_DIR, "shiao_nonimm_norm.h5ad")
OUT_PATH = os.path.join(NONIMM_DIR, "shiao_nonimm_norm_cc.h5ad")
...
# filter_genes pass, so cell cycle genes expressed only in immune cells are gone.
