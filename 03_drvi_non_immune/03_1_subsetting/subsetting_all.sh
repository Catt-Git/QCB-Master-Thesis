#!/usr/bin/env bash
#
# 03_1 subsetting: the headless half of the phase, in one command.
#
# Runs the four scripts that turn the re-filtered raw subset written by
# subset_and_qc.ipynb into the definitive non-immune object, in order:
#
#   1. scran_norm_nonimm.py       shiao_nonimm_raw.h5ad     -> shiao_nonimm_norm.h5ad
#   2. cell_cycle_score_nonimm.py shiao_nonimm_norm.h5ad    -> shiao_nonimm_norm_cc.h5ad
#   3. reduce_data_nonimm.py      shiao_nonimm_norm_cc.h5ad -> shiao_nonimm_reduced.h5ad
#                                                            + shiao_nonimm_hvg_2k_list.csv
#                                                            + shiao_nonimm_hvg_2k.h5ad
#   4. clustering_nonimm.py       shiao_nonimm_reduced.h5ad -> shiao_nonimm.h5ad
#
# Everything runs locally in sequence: 176k cells, so unlike 01_3 scran does not
...
# Logs go to 03_1_subsetting/logs/subsetting_all_<timestamp>.log as well as to
# the terminal.
...
STEP_SCRIPTS=(
  scran_norm_nonimm.py
  cell_cycle_score_nonimm.py
  reduce_data_nonimm.py
  clustering_nonimm.py
)
STEP_OUTPUTS=(
  shiao_nonimm_norm.h5ad
  shiao_nonimm_norm_cc.h5ad
  shiao_nonimm_reduced.h5ad
  shiao_nonimm.h5ad
)
...
INPUT_H5AD="$NONIMM_DIR/shiao_nonimm_raw.h5ad"
if [ ! -f "$INPUT_H5AD" ] && [ "$DRY_RUN" -eq 0 ]; then
  echo "missing $INPUT_H5AD" >&2
  echo "run subset_and_qc.ipynb first: it is the step that subsets the non-immune" >&2
  echo "cells, restores the raw counts and re-applies the gene/cell filters." >&2
  exit 1
fi

mkdir -p "$NONIMM_DIR" "$LOG_DIR"
LOG_FILE="$LOG_DIR/subsetting_all_$(date +%Y%m%d_%H%M%S).log"
[ "$DRY_RUN" -eq 0 ] && exec > >(tee -a "$LOG_FILE") 2>&1

echo "DATA_DIR  : $DATA_DIR"
echo "output dir: $NONIMM_DIR"
...
for i in "${!STEP_NAMES[@]}"; do
  ...
  output="$NONIMM_DIR/${STEP_OUTPUTS[$i]}"
...
echo "next: visualization_nonimm.ipynb on $NONIMM_DIR/shiao_nonimm.h5ad"
exit 0
