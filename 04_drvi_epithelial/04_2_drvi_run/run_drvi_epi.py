#!/usr/bin/env python
"""
04_2 DRVI run: the headless half of drvi_epi.ipynb.
...
on the cluster (submit_drvi_epi.slurm) or in a terminal and the notebook
...
and the same three outputs, named from the run id `drvi_epi_<n_latent>`:
    ...
    04_epi/shiao_epi_<run_id>.h5ad        the 04_1 object (all genes) + obsm['X_drvi']

Those three are everything this phase produces: 02_3 and 02_4 do not run on this
compartment, so nothing is written for the scib benchmark. 04_3 reads the
...
`shiao_epi.h5ad` to be around: if that object is missing the run says so up
front, writes the other two and skips it.

The default latent size is 64, the run of this phase, the same size as 03_2. 32
was tried first - half the cells (74,441 vs 176,610) and 10 labels of a single
lineage instead of 18 across four suggested a smaller space would do - and none
of its 32 dimensions vanished, the signal that the space is too tight, so the
default settled at 64. As in the notebook, this is a full re-run on the compartment and
inherits nothing from 03_2 or from the 02_2 whole-dataset run: `drvi_epi_*`,
`drvi_nonimm_*` and `drvi_unscaled_*` share the recipe and nothing else.

The notebook's figures are redrawn here too, into the same figures/04_2_<run_id>/
...
# names. `compartment` and `fraction` are not among them: after the 04_1 subset
# both are constant (`epi`, `non_imm`) and carry no information, and
# `dataset_origin` (the technical CD45 sort) is left out as everywhere in Part 2.
UMAP_QC_KEYS = {
    "cell_type": "cell_type",
    "cohort": "cohort",
    "treatment": "treatment",
...
def parse_args():
    p = argparse.ArgumentParser(description="Run DRVI on the epithelial compartment")
    ...
    p.add_argument("--fig-dir", default=None,
                   help="where the figures go [default: the repo's "
                        "04_drvi_epithelial/figures/04_2_<run_id>]")
...
def draw_figures(model, adata, embed, fig_dir, run_id):
    ...
    figures/04_1_visualization/umap_*_epi_unintegrated.png: the same cells in the
    *unintegrated* PCA space, with the same keys and the same palettes, so only
    ...
    embed_plot = ad.AnnData(
        ...
        # colour here and in the 04_1 UMAPs of the same cells.
    ...
    # One panel per cell type: three labels (Lumsec-prol, LummHR-major,
    # Lumsec-basal) are 81% of the compartment and dominate the single `cell_type`
    # panel, and where each population sits is the point - the more so here, where
    # the labels are sub-labels of one lineage and are expected to overlap.
...
def main():
    ...
    run_id = f"drvi_epi_{args.n_latent}"

    script_dir = Path(__file__).resolve().parent
    phase_dir = script_dir.parent                     # 04_drvi_epithelial/
    ...
    # The definitive object of 04_1 (all genes, scran log-norm, leiden): read once
    # at the end, only to carry the latent space over for 04_3.
    ...
    model_path = epi_dir / f"model_{run_id}.pt"        # one file (see save_model)
    latent_h5ad = epi_dir / f"embed_{run_id}.h5ad"     # latent space + interpretability
    downstream_h5ad = epi_dir / f"shiao_epi_{run_id}.h5ad"   # 04_1 object + obsm['X_drvi']

    fig_dir = Path(args.fig_dir) if args.fig_dir else phase_dir / "figures" / f"04_2_{run_id}"
    ...
    if not input_h5ad.exists():
        sys.exit(f"missing {input_h5ad}: run 04_1_subsetting/subsetting_all.sh first")
    ...
    # Sanity checks on what 04_1 produced: the epithelial compartment only, the
    # counts layer present. `compartment` is the column that tells this object from
    # the 03 one, where `fraction` is constant as well.
    ...
    # The number the latent size is judged on: how many of the n_latent dimensions
    # DRVI did not need. This is what to read in the log before deciding whether to
    # re-run at another size - none vanished means the size is too tight and a larger
    # one is worth a run, which is what the first attempt at 32 reported (0 / 32).
    ...
    # the definitive 04_1 object (all genes, scran log-norm, leiden) with the
    ...
    # other half, the compartment itself in the DRVI space. 04_3 as it stands
    ...
    if not full_h5ad.exists():
        print(f"[warn] {full_h5ad} is missing, so {downstream_h5ad.name} was not written.\n"
              f"       Run 04_1_subsetting/subsetting_all.sh (or copy that object here) and\n"
        ...
        # Cell order is not assumed: the latent space is realigned on the 04_1
    ...
    print("next: 04_3_enrichment on that embedding", flush=True)
