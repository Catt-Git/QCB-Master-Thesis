"""
04_1 scran normalization of the epithelial subset (mirrors 01_3).
...
factors, so removing the 544,241 non-epithelial cells changes every estimate.
The subset is therefore re-normalized from the raw counts restored by
`subset_and_qc.ipynb`, with the same parameters as 01_3 so the two objects stay
comparable.

Unlike 01_3 this runs locally: ~74k cells instead of 620k, no SLURM wrapper.

Input : $DATA_DIR/04_epi/shiao_epi_raw.h5ad
        Output of subset_and_qc.ipynb. .X = raw counts (int), identical to
        .layers['counts']; zero-count genes already removed.
Output: $DATA_DIR/04_epi/shiao_epi_norm.h5ad
...
IN_PATH = os.path.join(EPI_DIR, "shiao_epi_raw.h5ad")
OUT_PATH = os.path.join(EPI_DIR, "shiao_epi_norm.h5ad")
...
# script: 04_drvi_epithelial/figures/04_1_normalization/.
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
PHASE_DIR = os.path.dirname(SCRIPT_DIR)  # 04_drvi_epithelial/
FIG_DIR = os.environ.get("FIG_DIR", os.path.join(PHASE_DIR, "figures", "04_1_normalization"))
...
#    at this stage (cell_type has 10 levels, cohort 29).
...
ax.set_title("Size factors vs library size (epithelial)")
...
ax.set_title("Distribution of scran size factors (epithelial)")
fig.tight_layout()
