"""
04_1 feature selection + dimensionality reduction of the epithelial subset
(mirrors 01_5/scib_reduce_data.py), plus the DRVI input for 04_2.

The HVGs of phase 01 cannot be reused: they were selected on an object where
only 75,452 of 619,693 cells were epithelial, so the 2,000 genes in
`shiao_hvg_2k_unintegrated_list.csv` are dominated by immune and stromal
variation. Selecting again on the epithelial cells alone is the whole point of
this subsetting phase - it is what lets DRVI spend its latent dimensions on
luminal / basal biology instead of on lymphocyte and fibroblast programs. The
same argument applies against the 03 selection, made on a compartment that was
57% stromal / vascular.

Everything else matches 01_5 (same flavor, same 2,000 genes, same PCA), so the
epithelial object stays comparable with the full one.

No (re)normalization happens here: .X is consumed as-is from the previous step.

Input : $DATA_DIR/04_epi/shiao_epi_norm_cc.h5ad
        .X       = scran log-normalized (from scran_norm_epi.py)
        .layers['counts'] = raw integer counts
        .var     = gene symbols as var_names, Ensembl in .var['gene_ids']
        .obs     = 'cohort' (29 patients, integration batch_key), 'cell_type'
                   (CellTypist majority_voting label_key), etc.
Output: $DATA_DIR/04_epi/shiao_epi_reduced.h5ad
        adds .var['highly_variable'] (binary, batch-aware), .obsm['X_pca'],
        neighbors graph, .obsm['X_umap']
        $DATA_DIR/04_epi/shiao_epi_hvg_2k_list.csv
        gene symbols of the selected HVGs, one per line, no header
        $DATA_DIR/04_epi/shiao_epi_hvg_2k.h5ad
        the same object restricted to those 2,000 genes, raw counts preserved in
        .layers['counts'] -- this is what DRVI trains on in 04_2
...
output already exists, and tees everything to 04_1_subsetting/logs/.
...
IN_PATH = os.path.join(EPI_DIR, "shiao_epi_norm_cc.h5ad")
OUT_PATH = os.path.join(EPI_DIR, "shiao_epi_reduced.h5ad")
HVG_CSV_PATH = os.path.join(EPI_DIR, "shiao_epi_hvg_2k_list.csv")
HVG_H5AD_PATH = os.path.join(EPI_DIR, "shiao_epi_hvg_2k.h5ad")
...
FIG_DIR = os.environ.get("FIG_DIR", os.path.join(PHASE_DIR, "figures", "04_1_reduce_data"))
...
# The same against the 03 non-immune selection, when that chain has been run: it says how much
# of the non-immune HVG budget was already epithelial, i.e. what this phase buys over 03.
nonimm_hvg_path = os.path.join(DATA_DIR, "03_nonimm", "shiao_nonimm_hvg_2k_list.csv")
if os.path.exists(nonimm_hvg_path):
    nonimm_hvg = set(pd.read_csv(nonimm_hvg_path, header=None)[0].astype(str))
    shared_nonimm = len(set(hvg_genes) & nonimm_hvg)
    print(f"Overlap with the 03_1 non-immune HVGs : {shared_nonimm}/{N_HVGS} "
          f"({100 * shared_nonimm / N_HVGS:.1f}%)", flush=True)

# Diagnostic figures
sc.pl.pca_variance_ratio(adata, n_pcs=50, log=True, save="_epi_elbow_unintegrated.png")
sc.pl.pca(adata, color=LABEL_KEY, save="_epi_cell_type_unintegrated.png")
sc.pl.umap(adata, color=LABEL_KEY, save="_epi_cell_type_unintegrated.png")
sc.pl.umap(adata, color=BATCH_KEY, save="_epi_cohort_unintegrated.png")
if "phase" in adata.obs:
    sc.pl.umap(adata, color="phase", save="_epi_phase_unintegrated.png")
