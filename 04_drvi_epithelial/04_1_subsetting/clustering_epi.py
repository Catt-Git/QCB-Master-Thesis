"""
04_1 leiden clustering at the optimal resolution (mirrors 01_5/scib_clustering.py).
Produces the definitive epithelial object.

Kept separate from reduce_data_epi.py because this is the stochastic,
...
48 CellTypist labels; here only 10 are observed, but they are the sub-labels of a
single lineage (LummHR-*, Lumsec-*, basal), so the structure leiden has to
resolve is finer than that count suggests and the NMI optimum can sit above 1.0.
Cutting the grid at 1.0 would hide it.

Input : $DATA_DIR/04_epi/shiao_epi_reduced.h5ad
        expects the neighbors graph already computed (from reduce_data_epi.py)
Output: $DATA_DIR/04_epi/shiao_epi.h5ad
        adds per-resolution leiden columns 'optscib_epi_leiden_<res>', the
        selected 'optscib_epi_leiden' and the resolution/NMI profile in
        .uns['optscib_epi_leiden_profile']
        $DATA_DIR/04_epi/shiao_epi_leiden_resolution_profile.csv
...
output already exists, and tees everything to 04_1_subsetting/logs/. Since this
step is the one most likely to be rerun on its own, note that re-running it
needs --force, otherwise the existing shiao_epi.h5ad marks it as done.
...
LABEL_KEY = "cell_type"
CLUSTER_KEY = "optscib_epi_leiden"

DATA_DIR = os.environ["DATA_DIR"]
EPI_DIR = os.path.join(DATA_DIR, "04_epi")
IN_PATH = os.path.join(EPI_DIR, "shiao_epi_reduced.h5ad")
OUT_PATH = os.path.join(EPI_DIR, "shiao_epi.h5ad")  # definitive epithelial object
PROFILE_CSV_PATH = os.path.join(EPI_DIR, "shiao_epi_leiden_resolution_profile.csv")

adata = sc.read_h5ad(IN_PATH)
print(f"Loaded {adata.n_obs:,} cells x {adata.n_vars:,} genes", flush=True)

assert "neighbors" in adata.uns, "neighbors graph missing; run reduce_data_epi.py first"
...
# per-resolution column ('optscib_epi_leiden_<res>') for the visualization grid.
