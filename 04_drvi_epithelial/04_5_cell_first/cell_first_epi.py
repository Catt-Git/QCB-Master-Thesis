...
How it fails, and what this script does about it:
  ...
  * the stemness lists are embryonic and proliferation-heavy while this compartment has a
    large Lumsec-prol population, so "stem-high" can just mean "cycling" -> the target set is
    recomputed inside G1 alone and the two cell sets compared. This check runs for every
    collection: an EMT score is as capable of tracking the cycle as a stemness one;
  * immune evasion is defined by ABSENCE of signal, which shallow sequencing mimics
    ...
    (`risks = ("depth", ...)`);
  * in an epithelial compartment a high mesenchymal score is as easily fibroblast ambient RNA
    or an epithelial-fibroblast doublet as it is a transition -> the doublet score of the
    mesenchymal-high group is compared against the rest (`risks = ("ambient", ...)`);
...
def main():
    ...
    if "CytoTRACE2" in coll.extra_readouts and C.CYTOTRACE_CSV.exists():
        ...
        print(f"\n[warn] {C.CYTOTRACE_CSV} not found - CytoTRACE2 is left out of the "
              ...
              "       Run cytotrace2_epi.py from the `cytotrace2-py` env (see its header; "
    ...
    C.banner("A3 - standardisation within (cohort, cell_type)")
    ...
    if coll.has_target:
        ...
        #
        # This compartment was subset to epithelial cells, but subsetting removes cells, not the
        # fibroblast transcripts that leaked into their droplets. VIM / FN1 / SPARC / ACTA2 high is
        # therefore the expected signature of contamination as much as of a transition, and without
        # this check an EMT result cannot be told apart from a soup result. `doublet_score` comes
        # from 04_1 (Scrublet) and is not recomputed here.
        if "ambient" in coll.risks and coll.ambient_risk_axis:
            ...
            print(f"\n{coll.ambient_risk_axis}-high vs the rest, doublet_score (Scrublet, from 04_1):")
            for r in on_axis:
    ...
    # The effect size of the consensus quadrant on each dimension. 04_7 reads this table
    ...
    if coll.has_target:
        ...
        for a, pl in zip(axes.ravel(), planes):
            ...
            a.set_xlabel(f"z {pl.x} ({pl.x_rule}) (within cohort x cell_type)", fontsize=8)
    ...
    # SIGN of rho, exactly as 04_7 reads it when it joins the two routes - rho > 0 on DR 7 is
