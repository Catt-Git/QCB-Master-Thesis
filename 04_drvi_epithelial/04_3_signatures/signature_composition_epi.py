#!/usr/bin/env python3
"""04_3b: what is actually inside each signature score - which genes carry it, and are they measurable.
...
This step opens the lists and reports, per gene and per signature:
  ...
  * `spearman_with_score`      - the gene against the score the rest of the phase actually uses,
                                 joined from 04_5's `signature_scores_*.csv` when it exists.
...
Runs AFTER 04_5 in the driver chain, only so it can join the real scores; without them it still
...
# Matched to MIN_SIGNATURE_GENES, which is the floor 04_3 already applies to the raw count:
...
def parse_args():
    ...
    p.add_argument("--no-spearman", action="store_true",
                   help="skip the per-gene correlation with the 04_5 score (the slow part)")
    return p.parse_args()
...
def spearman_against(scores: np.ndarray, X: sp.spmatrix, chunk: int = 250) -> np.ndarray:
    ...
    74,441 cells x 1,698 genes in float64 is 1 GB, and every list is scored separately.
    """
    y = rankdata(scores)
    y = (y - y.mean()) / y.std()
...
def main():
    ...
    # ------------------------------------------------------------------ scores
    # Joined when 04_5 has run, exactly as 04_5 joins CytoTRACE2 when 04_4 has: the column is
    # worth having and nothing here depends on it, so its absence is reported, not fatal.
    scores = None
    if args.no_spearman:
        print("[note] --no-spearman: skipping the per-gene correlation with the 04_5 score")
        ...
        print(f"[note] {C.scores_csv(coll).name} not found - run 04_5 first for the "
    ...
    if scores is not None:
        if list(scores.index) != list(adata.obs_names):
            sys.exit("[STOP] the 04_5 score table is not in the object's cell order.")
    ...
    C.savefig("signature_composition", "04_3_signatures", coll, fig, caveat=False)
    plt.close(fig)
